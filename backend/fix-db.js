// prettier-ignore
/* eslint-disable */
// This file uses only JS unicode escapes (\uXXXX) — no literal accented chars
// so it stays ASCII-safe regardless of editor encoding settings.
const fs = require('fs');
...
// Delete stale db.json so fresh SEED is used
if (fs.existsSync("data/db.json")) fs.unlinkSync("data/db.json");

// Escape all non-ASCII chars to \uXXXX so db.js stays pure ASCII
function escapeNonAscii(str) {
  return str.replace(/[^\x00-\x7F]/g, (c) => {
    return "\\u" + c.charCodeAt(0).toString(16).padStart(4, "0");
  });
}
const seedJsonAscii = escapeNonAscii(JSON.stringify(seed, null, 2));
...
fs.writeFileSync("src/db.js", dbSrc, "utf8");

// Verify
delete require.cache[require.resolve("./src/db")];
const db = require("./src/db");
const rows = db.read().actualites;
const code = rows[1].categorie.charCodeAt(0);
process.stdout.write(
  "charcode[0] of categorie[1]: " +
    code +
    (code === 201 ? " OK" : " FAIL") +
    "\n",
);
process.stdout.write("titre[2]: " + rows[2].titre + "\n");
process.stdout.write("titre[5]: " + rows[5].titre + "\n");
